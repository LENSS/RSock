...
namespace hrp {
    ...
    public:
        explicit HrpAPIMultiplexer(HrpRouter &router) : _router(router) {
            ...
            if (!_logger) _logger = std::make_shared<spdlog::logger>("HrpAPIMultiplexer", file_sink);
            if (debug_mode)
    ...
    public:
        HrpClientConnection(asio::ip::tcp::socket sock, HrpAPIMultiplexer &multiplexer)
                : socket_(std::move(sock)), _multiplexer(multiplexer), _recv_buf(HRP_DEFAULT_DATA_BUF_SIZE),
                  _send_buf(HRP_DEFAULT_DATA_BUF_SIZE)
        {
            _logger = spdlog::get("HrpClientConnection");
            if (!_logger) _logger = std::make_shared<spdlog::logger>("HrpClientConnection", file_sink);
            if (debug_mode)
                _logger->set_level(spdlog::level::debug);
        }
    ...
    public:
        HrpAPIServer(asio::io_service &io_service, unsigned short port, HrpRouter &router)
                : _io_service(io_service), _port(port), _multiplexer(router),
                acceptor_(io_service, asio::ip::tcp::endpoint(asio::ip::address::from_string("127.0.0.1"), port)),
                socket_(io_service) {
            _logger = spdlog::get("HrpAPIServer");
            if (!_logger) _logger = std::make_shared<spdlog::logger>("HrpAPIServer", file_sink);
            if (debug_mode)
                _logger->set_level(spdlog::level::debug);
