...
namespace hrp {
    ...
    std::vector<double> calcDelays(unsigned int rmax, const MVECore &_srcCore, const MVECore &_dstCore) {
        if (rmax == 0) throw hrpException("Invalid rmax: cannot be zero");
        ...
        if (total_rate_to_carriers == 0) {
            ...
            if (total_direct_rate_to_dst > 0) ret.assign(rmax, 1/total_direct_rate_to_dst);
            else ret.assign(rmax, std::numeric_limits<double>::max());
            return ret;
        }
        ...
        for (auto itr = _dstCore.get_allNodes().begin(); itr != _dstCore.get_allNodes().end(); itr++) {
            ...
            if (*itr == _srcCore.get_thisNode()) {
                ...
                for (auto mapitr = _srcCore.get_params().begin(); mapitr != _srcCore.get_params().end(); mapitr++) {
                    if (mapitr->first.find(*itr) != mapitr->first.end()
                            && mapitr->first.find(_dstCore.get_thisNode()) == mapitr->first.end()) {
                        // Note, r might be zero in some cases
                        double r = mapitr->second;
                        ect_set intersect_set;
                        std::set_intersection(mapitr->first.begin(), mapitr->first.end(),
                                              _dstCore.get_allNodes().begin(), _dstCore.get_allNodes().end(),
        ...
        for (unsigned int repl = 2; repl <= rmax; repl++ ) {
            ...
            if (total_rate_to_dst == 0) ret[repl-1] = std::numeric_limits<double>::max();
            else ret[repl-1] = (repl - 1)/total_rate_to_carriers + (1.0 / total_rate_to_dst);
