...
namespace hrp {
    ...
    double RMCore::calcLoss(double alpha, unsigned int repl, unsigned int rmax, double latency, double one_hop_latency) {
        if (latency < 0) throw hrpException("Invalid latency value: negative");
        if (one_hop_latency < 0) throw hrpException("Invalid one_hop_latency value: negative");
        if (repl > rmax) throw hrpException("Invalid repl value: cannot be larger than rmax");
