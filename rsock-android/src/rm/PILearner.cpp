...
namespace hrp {
    PILearner::PILearner(RMCore &core, double alpha) : _core(core), _alpha(alpha) {
        ...
        if (!_logger) _logger = std::make_shared<spdlog::logger>("PILearner", file_sink);
        if (debug_mode)
...
//                }
    ...
    unsigned int PILearner::drawNextAction() {
        unsigned int ret;
        if (_pi_action < _core.get_rmax()) ret = _pi_action + 1;
        else ret = _core.drawAction();
    ...
    PILearner::get_out_order_buf() const {
