...
namespace hrp
{
	...
	void HrpBloomFilterMessage::init_from_proto_msg()
	{
		...
		for (int i=0; i<_proto_msg.salt__size(); i++)
			salt_.push_back(_proto_msg.salt_(i));

		const std::string& bit = _proto_msg.bit_table_();
		
		for (int i=0; i<_proto_msg.bit_table_().length(); i++)
			bit_table_.push_back((unsigned char)bit[i]);
	}
