...
namespace hrp
{
	...
	MVECore::MVECore(const MVECore &core) :
	_params(core._params), _allNodes(core._allNodes), _thisNode(core._thisNode)
	{
	}
	...
	{
		if (v < 0)
			throw hrpException("Invalid MVE parameter (negative) for contact rate.");
		_params[k] = v;
