...
namespace hrp {
	...
	{
		if (length < 0)
			throw hrpException("HrpGraph::addArc, length < 0");
	...
	*/
	...
	std::vector<HrpGraph::hrpPathEntry> HrpGraph::findKShortestPath(const hrpNode &src, const hrpNode &dst, int K)
	{
		...
		*/
		/*
		_logger->debug("call HrpGraph::findKShortestPath for");
		_logger->debug(printHrpNode("src ", src));
		_logger->debug(printHrpNode("dst ", dst));
		*/
		if (src.size() == 0 || dst.size() == 0 || _vertices.count(src) == 0 || _vertices.count(dst) == 0)
		{
			string error = string("HrpGraph::findKShortestPath Invalid argument, src or dst is not in the graph") + printHrpNode("",src) +  printHrpNode("",dst);
			throw hrpException(error);
		}
		...
		auto compare = [] (hrpPathEntry e1, hrpPathEntry e2)
		{
		...
		//_logger->debug("call dijkstra for: {}, {}", printHrpNode("",src), printHrpNode("",dst));

		
		// If no path found
		if (p1.second == HRPGRAPH_UNREACHABLE_PATH)
		{
			// Commented and Replaced by Ala
			/*
			...
			*/
			...
			//_logger->error("HrpGraph::unrechable path when call dijkstra for: {}, {}", printHrpNode("",src), printHrpNode("",dst));	
			// End Ala

			string error = string("HrpGraph::unrechable path when call dijkstra for src:" + printHrpNode("",src) + " dst:" +  printHrpNode("",dst));
			throw hrpException(error);
		}
		...
		if (A.empty())
			throw hrpException("HrpGraph::findKShortestPath no route to dst");
		return A;
	}
	...
	HrpGraph::succMap HrpGraph::convertKPathToSuccMap(const std::vector<HrpGraph::hrpPathEntry> &A)
	{
		...
		for (hrpPathEntry entry : A)
		{
			...
			{
				if (retmap.count(path[i]) == 0)
					retmap[path[i]] = std::set<hrpNode>();
	...
	std::string HrpGraph::pretty_print() const
	{
		...
		*/
		...
		for (auto itr = _vertices.begin(); itr != _vertices.end(); itr++)
		{
			ret += printHrpNode("\t",itr->first) + "\n";
		}
		ret += "\nedges: \n";
		for (lemon::ListDigraph::ArcIt it(_graph); it != lemon::INVALID; ++it)
		{
			ret += printHrpNode("\t",_vertexNames[_graph.source(it)]) + " -> " + printHrpNode("",_vertexNames[_graph.target(it)]) + " lq: " + to_string(_arcLength[it]) + "\n";
		}
		return ret;
