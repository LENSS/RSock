...
namespace hrp
{
	class TxrxEngine
	{
		...
		*/
		explicit TxrxEngine(asio::io_service &io_service, unsigned short port, TopologyManager *);
		// End Ala
		void init();
		void shutdown();
		...
		*/
		...
		private:
		std::vector<std::string> get_node_IPs(hrpNode& node);
		void run_io_service();
		void start_receive();
		void handle_receive(const asio::error_code &error, std::size_t size);
		void handle_send(std::shared_ptr<std::string>, std::shared_ptr<HrpDataHeader> header, std::function<void(std::shared_ptr<HrpDataHeader>, bool)> cb,const asio::error_code &error, std::size_t transferred_size);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	class NeighborDiscoveryAgent
	{
		...
		*/
		explicit NeighborDiscoveryAgent(const hrpNode addr, asio::io_service &io_service) :
		// End Ala
		_addr(addr),
		...
		*/
		std::map<hrpNode, std::chrono::steady_clock::time_point> _peers;
		// End Ala
