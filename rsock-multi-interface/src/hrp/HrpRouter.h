...
namespace hrp
{
	...
	class HrpRouter
	{
		...
		*/
		...
		// Added by Ala to send buffered packet frequently to connected nodes
		
		void clearBufferMsgs(const std::error_code &error);
		void scheduleclearBufferMsgsTimer();
		asio::steady_timer  *_checkBuffer_timer;
		/**
		* Traverse all buffered messages and check if there's any message can
		* be forwarded
		*/
		void tryBufferedMessages();	
