...
namespace hrp
{
	...
	void HrpAPIClient::do_connect()
	{
		...
		[this](const std::error_code &ec)
		{
			...
			{
				std::cout << "App successfully connected to daemon" << std::endl;
				do_receive();
			}
			_cv.notify_all();
		});
	...
	void HrpAPIClient::handle_receive(const std::error_code &ec, size_t size)
	{
		if (!ec)
		{
			//std::cout << "HrpAPIClient::handle_receive() Start "<<endl;
			...
			//std::cout << "HrpAPIClient::handle_receive() After asio::async_read... "<<endl;
	...
	long HrpAPIClient::recv(void *buf, size_t max_size, hrpNode &src, long &delay)
	{
		...
		//std::cout << "HrpAPIClient::pkt.getSz_of_payload()):" << pkt.getSz_of_payload()<<endl;
		//std::cout << "HrpAPIClient::pkt.getPayload():" << pkt.getPayload()<<endl;

		src = pkt.getNode();
		// Added by Ala
		/*
		if(src.size() == 1)
			std::cout << "HrpAPIClient::recv guid:" << src.at(0);
		if(src.size() == 2)
			std::cout << "HrpAPIClient::recv Ip1:" << src.at(1);
		if(src.size() == 3)
			std::cout << "HrpAPIClient::recv Ip2:" << src.at(2);
		*/
		// End Ala
		delay = pkt.getTime();
		delete []pkt.getPayload();
	...
	// Added by Ala
	long HrpAPIClient::requestTopology()
	{
		uint32_t buf_sz;
		auto *to_send = HrpAPIPacketHelper::requestTopology(buf_sz);
		if (buf_sz > HRP_DEFAULT_DATA_BUF_SIZE)
		{
			std::cout << "HrpAPIClient::requestTopology cannot send data larger than " << HRP_DEFAULT_DATA_BUF_SIZE << " bytes\n";
			return -1;
		}
...
/*
	...
	_socket(_io_service, asio::ip::udp::endpoint(asio::ip::udp::v4(), _port)),
	...
	void OlsrTestAPI::handle_receive(const std::error_code &error, std::size_t size)
	{
		if (!error)
		{
			...
			else
			{
				...
				_socket.async_send_to(asio::buffer(_send_buf.data(), size),_remote_endpoint,
				std::bind(&OlsrTestAPI::handle_send,this,
